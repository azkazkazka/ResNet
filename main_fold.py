...
def load_and_predict(best_model_path, X_test, y_test, file_names, scenario):
    ...
    with open(f'./best_model/{scenario}/test_predictions_test_only_prosa_to_cv.txt', 'w') as f:
        for idx, prediction in enumerate(predictions):
            filename = file_names[idx]
            f.write(f"{filename} - Predicted: {prediction[0]:.1f} - Actual: {y_test[idx]}\n")
    print(f"Predictions logged successfully to ./best_model/{scenario}/test_predictions_test_only_prosa_to_cv.txt")
...
def train_model(args):
    ...
    train_list = os.path.join(base_path, "train.lst")
    val_list = os.path.join(base_path, "val.lst")
    ...
    try:
        ...
        print("Start trainval preprocess")
        ...
        for e in epochs:
            for train_index, val_index in kf.split(features):
                ...
                _, _ = train_and_evaluate_model(i, X_train, y_train, X_val, y_val, audio_files, scenario, epochs=e, batch_size=batch_size[0], learning_rate=learning_rate[0], n_splits=fold)
                i += 1
...
def evaluate_model(args):
    ...
    protocol_path = os.path.join(base_path, "protocol.txt")
    test_list = os.path.join(base_path, "test_cv_only.lst")
    ...
    try:
        ...
        predictions_file = f"./best_model/{scenario}/test_predictions_test_only_prosa_to_cv.txt"
