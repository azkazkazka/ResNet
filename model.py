...
def train_and_evaluate_model(X_train, y_train, file_names_train, X_val, y_val, file_names_val, scenario, batch_size=32, epochs=100, learning_rate=0.001, n_splits=5, save_path='model_per_fold'):
    ...
    try:
        ...
        print("Immediate history keys:", history.history.keys())
        print("Sample values:", {k: history.history[k] for k in history.history.keys()})
    ...
    history_file_path = f'./model_histories/{scenario}_histories.pkl'

    # Save all_histories to a file
    with open(history_file_path, 'wb') as file:
        pickle.dump(all_histories, file)

    print(f"Saved training histories to {history_file_path}")
