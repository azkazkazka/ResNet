...
def calculate_accuracy(cm_scores, cm_keys, threshold=0.5):
    """
    Calculate accuracy of the classification based on a given threshold.
    
    Parameters:
    cm_scores (np.array): Array of scores.
    cm_keys (np.array): Array of true labels (0 or 1).
    threshold (float): Decision threshold to classify scores.
    
    Returns:
    float: Accuracy of the classification.
    """
    predictions = (cm_scores > threshold).astype(int)
    accuracy = accuracy_score(cm_keys, predictions)
    return accuracy

def generate_confusion_matrix(cm_scores, cm_keys, threshold=0.5):
    """
    Generate confusion matrix based on a given threshold.
    ...
    """
    predictions = (cm_scores > threshold).astype(int)
    cm = confusion_matrix(cm_keys, predictions)
    return cm
